import 'package:edemand_partner/ui/screens/withdrawal/enum.dart';
...
class WithdrawalRequestsScreenState extends State<WithdrawalRequestsScreen>
  ...
  final List<String>
      statuses = /* ['all', 'pending', 'approved', 'rejected', 'settled'] */
      WithDrawalStatusEnum.values.map((e) => e.name).toList();
  ...
  void fetchWithDrawalRequest() {
    context
        .read<FetchWithdrawalRequestCubit>()
        .fetchWithdrawals(status: selectedStatus);
  }
  ...
  void _pageScrollListen() {
    ...
    if (_pageScrollController.isEndReached()) {
      if (context.read<FetchWithdrawalRequestCubit>().hasMoreData()) {
        context
            .read<FetchWithdrawalRequestCubit>()
            .fetchMoreWithdrawals(status: selectedStatus);
      }
    }
  }
    ...
    return Stack(
      children: [
        CustomRefreshIndicator(
          ...
          child: BlocConsumer<FetchWithdrawalRequestCubit,
              FetchWithdrawalRequestState>(
            listener: (context, state) {
              if (state is FetchWithdrawalRequestSuccess) {
                // update amount globally
                context
                    .read<FetchSystemSettingsCubit>()
                    .updateAmount(state.availableBalance);
              }
            },
            builder: (BuildContext context, FetchWithdrawalRequestState state) {
                ...
                return state.withdrawals.isEmpty
                    ? Center(
                        child: NoDataContainer(
                            titleKey:
                                'noDataFound'.translate(context: context)),
                      )
              ...
              return ListView.builder(
                ...
                itemCount: 9, // Increased by 1 to account for the SizedBox
                physics: const NeverScrollableScrollPhysics(),
                clipBehavior: Clip.none,
                itemBuilder: (BuildContext context, int index) {
                  if (index == 0) {
                    return const CustomSizedBox(height: 130);
                  }

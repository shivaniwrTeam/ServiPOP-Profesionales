...
class MainActivityState extends State<MainActivity>
  ...
  Widget _buildDraggableItem(int index) {
    return LongPressDraggable<int>(
      data: index,
      key: ValueKey(gridItems[index].name), // Unique Key
      ...
      child: DragTarget<int>(
        ...
        builder: (context, candidateData, rejectedData) {
          return GestureDetector(
            onTap: () {
              onClickOfDrawerItems(gridItems[index].name);
            },
            child: TweenAnimationBuilder<double>(
              duration: const Duration(milliseconds: 200),
              curve: Curves.easeOut,
              tween:
                  Tween<double>(begin: _draggedItemSize, end: _draggedItemSize),
  ...
  void onClickOfDrawerItems(String name) {
    if (name == 'promoCodeLbl') {
      ...
      final List<Map<String, dynamic>> values = appLanguages.map((element) {
        return {
      ...
      UiUtils.showModelBottomSheets(
        ...
        if (value != null) {
          context.read<LanguageCubit>().changeLanguage(value["selectedItemId"]);
        }
      });
      ...
      if (Platform.isAndroid) {
        ...
        Share.share(context.read<FetchSystemSettingsCubit>().getPlayStoreURL());
      } else {
        // Set a small valid popover rectangle (centered)
        final Rect sharePosition = Rect.fromLTWH(
            0,
            0,
            MediaQuery.of(context).size.width,
            MediaQuery.of(context).size.height / 2);
        Share.share(context.read<FetchSystemSettingsCubit>().getAppStoreURL(),
            ...
            AnimatedContainer(
              ...
              child: BackdropFilter(
                ...
                child: CustomContainer(
                    color:
                        context.colorScheme.blackColor.withValues(alpha: .2)),
              ),
            ),
          AnimatedPositioned(
            ...
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 500),
              curve: Curves.easeInOut,
              width: _showOverlay ? MediaQuery.of(context).size.width * 0.9 : 0,

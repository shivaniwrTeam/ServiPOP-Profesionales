...
class FetchServicesCubit extends Cubit<FetchServicesState> {
  ...
  Future<void> fetchMoreServices({String? order, String? sort}) async {
    try {
      if (state is FetchServicesSuccess) {
        ...
        emit((state as FetchServicesSuccess)
            .copyWith(isLoadingMoreServices: true));

        final DataOutput<ServiceModel> result =
            await _serviceRepository.fetchService(
  ...
  Future<void> searchService(String query, String? order, String? sort) async {
    try {
      ...
      final DataOutput<ServiceModel> result =
          await _serviceRepository.fetchService(
              offset: 0, searchQuery: query, order: order, sort: sort);
  ...
  bool hasMoreServices() {
    if (state is FetchServicesSuccess) {
      return (state as FetchServicesSuccess).offset <
  ...
  void editService(ServiceModel updatedModel) {
    if (state is FetchServicesSuccess) {
      final List<ServiceModel> services =
          List.from((state as FetchServicesSuccess).services);

      final int index =
          services.indexWhere((service) => service.id == updatedModel.id);

  ...
  Future<void> fetchMoreJobRequest(String? jobType) async {
    try {
      if (state is FetchJobRequestSuccess) {
        ...
        emit((state as FetchJobRequestSuccess)
            .copyWith(isLoadingMoreJobRequest: true));

        final DataOutput<JobRequestModel> result =
            await _jonRequestRepository.fetchCustomJobRequest(
                offset:
                    (state as FetchJobRequestSuccess).offset + UiUtils.limit,
                jobType: jobType);
